...
### 题目1：请分析检材1：手机的序列号是什么？

- 题目原文：`请分析检材1：手机的序列号是什么？`
- 证据类型：`手机取证 / Android 系统日志`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/logs/adb/ksu/log/dmesg.log`；`exports/logs/adb/ksu/log/dmesg.old.log`
- 关键证据/关键路径：`dmesg.log` 中 `serialno=` 和 `ro.boot.serialno`
...
- 推理：系统启动日志里已经直接给出 `serialno=AM69VKKVU4CMGELB`，并且只读属性 `ro.boot.serialno` 的写入报错再次出现同一值，可直接确认设备序列号。
- 最终答案：`AM69VKKVU4CMGELB`

### 题目2：请分析检材1：该手机的具体型号是什么？

- 题目原文：`请分析检材1：该手机的具体型号是什么？`
- 证据类型：`手机取证 / Android 系统属性`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/props/adb/lspd/log/props.txt`；`exports/props/adb/lspd/log.old/props.txt`
- 关键证据/关键路径：`props.txt` 中 `ro.product.marketname` 与 `ro.product.model`
- 提取/查询方法：检索 `ro.product.marketname` 和 `ro.product.model`
- 重要截图：无
- 摘录：
...
- 推理：厂商市场名字段直接给出 `一加Ace 5 至尊版`，型号字段为 `PLC110`；题目要求具体型号，WP 统一写作 `一加Ace5至尊版`。
- 最终答案：`一加Ace5至尊版`

### 题目3：请分析检材1：手机中已删除的联系人手机号码是什么？

- 题目原文：`请分析检材1：手机中已删除的联系人手机号码是什么？`
- 证据类型：`手机取证 / 联系人数据库`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/contacts/data/com.android.providers.contacts/databases/contacts2.db`
- 关键证据/关键路径：表 `raw_contacts`、`data`、`mimetypes`；`raw_contacts.deleted=1`
- 提取/查询方法：关联 `raw_contacts`、`data`、`mimetypes`，筛选 `raw_contacts.deleted=1`
- 重要截图：无
- 摘录：
...
- 推理：该联系人记录已被标记删除，且 `phone_v2` 类型的 `data1` 字段即联系人手机号，因此可恢复出已删除手机号 `13696966666`。
- 最终答案：`13696966666`

### 题目4：请分析检材1：手机中找到密码本文件，计算其MD5哈希值，取后6位，字母大写。

- 题目原文：`请分析检材1：手机中找到密码本文件，计算其MD5哈希值，取后6位，字母大写。`
- 证据类型：`手机取证 / 文件哈希`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/selected_from_tar/media/0/常用密码.txt`
- 关键证据/关键路径：文件 `常用密码.txt`
...
- 推理：密码本文件 `常用密码.txt` 的 MD5 为 `B159BD42F13027EF4A5637535FAD3563`，按题意截取末 6 位并大写得到 `AD3563`。
- 最终答案：`AD3563`

### 题目5：请分析检材1：嫌疑人下载的图片隐写工具名称是什么？

- 题目原文：`请分析检材1：嫌疑人下载的图片隐写工具名称是什么？`
- 证据类型：`手机取证 / 下载文件`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/selected_from_tar/media/0/Download/tgllq/steghide-0.5.1-win32.zip`
- 关键证据/关键路径：`media/0/Download/tgllq/steghide-0.5.1-win32.zip`
- 提取/查询方法：在原始 tar 与抽取目录中检索 `steghide`
- 重要截图：无
- 摘录：
...
- 推理：下载目录中直接存在压缩包 `steghide-0.5.1-win32.zip`，文件名本身已表明所下载的图片隐写工具名称。
- 最终答案：`steghide-0.5.1-win32.zip`

### 题目6：请分析检材1：嫌疑人使用隐写工具进行文件隐写密码是什么？

- 题目原文：`请分析检材1：嫌疑人使用隐写工具进行文件隐写密码是什么？`
- 证据类型：`手机取证 / 隐写文件与密码本`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/selected_from_tar/media/0/DCIM/Camera/important.jpg`；`exports/selected_from_tar/media/0/常用密码.txt`
- 关键证据/关键路径：`important.jpg` 的 steghide 信息；密码本候选值 `JHTJ@202605`
- 提取/查询方法：使用密码本候选值执行 `steghide.exe info important.jpg -p JHTJ@202605`
- 重要截图：无
- 摘录：
...
- 推理：当密码取 `JHTJ@202605` 时，`steghide` 能正常识别 `important.jpg` 中嵌入的 `重点客户.z01`，说明该值就是正确隐写密码。
- 最终答案：`JHTJ@202605`

### 题目7：请分析检材1：嫌疑人向数据贩子购买公民个人信息共计多少条？

- 题目原文：`请分析检材1：嫌疑人向数据贩子购买公民个人信息共计多少条？`
- 证据类型：`手机取证 / 即时通信数据库`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/chat_lianqu/data/com.lianqujiaoyou.chat/app_flutter/OpenIM_v3_1355997814.db`
- 关键证据/关键路径：表 `chat_logs_si_1082423674_1355997814`，`rowid=10`，`seq=17`
- 提取/查询方法：检索聊天内容中的 `100` 与 `富豪`
- 重要截图：无
- 摘录：
...
- 推理：嫌疑人在聊天中直接提出“先给我来个100条富豪的数据”，该数值即本次购买的个人信息条数。
- 最终答案：`100`

### 题目8：请分析检材1：嫌疑人向数据贩子交易时使用的付款方式是什么？

- 题目原文：`请分析检材1：嫌疑人向数据贩子交易时使用的付款方式是什么？`
- 证据类型：`手机取证 / 聊天记录`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/app_uni04963c4/data/uni.app.UNI04963C4/files/1600123285_3137373036323233393731/msg_0.db`
- 关键证据/关键路径：表 `message`，`id=7` 至 `id=10`
- 提取/查询方法：读取 `id,time,is_self,sender,element_descriptions`
- 重要截图：无
- 摘录：
...
- 推理：对方在付款方式选项中明确回复“银行卡吧”，后续又给出具体工行卡号，因此交易付款方式为银行卡。
- 最终答案：`银行卡`

### 题目9：请分析检材1：数据贩子交易过程中提供的银行卡号是多少？

- 题目原文：`请分析检材1：数据贩子交易过程中提供的银行卡号是多少？`
- 证据类型：`手机取证 / 聊天记录`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/app_uni04963c4/data/uni.app.UNI04963C4/files/1600123285_3137373036323233393731/msg_0.db`
- 关键证据/关键路径：表 `message`，`id=10`
- 提取/查询方法：定位交易收款信息所在消息记录
- 重要截图：无
- 摘录：
...
- 推理：聊天消息直接给出完整工行卡号 `6222355973482923738`，即交易过程中提供的银行卡号。
- 最终答案：`6222355973482923738`

### 题目10：请分析检材1：接上题，嫌疑人所使用聊天工具的用户 ID 是多少？

- 题目原文：`请分析检材1：接上题，嫌疑人所使用聊天工具的用户 ID 是多少？`
- 证据类型：`手机取证 / 应用配置数据库`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/app_uni04963c4/data/uni.app.UNI04963C4/databases/DCStorage`
- 关键证据/关键路径：表 `DC_1046388429_storage`，`rowid=60`，`key=userInfo`
- 提取/查询方法：读取 `userInfo` 并与聊天消息 `sender` 字段交叉验证
- 重要截图：无
- 摘录：
...
- 推理：应用存储中的 `userInfo.userID` 为 `17706223971`，且聊天数据库里该 ID 对应嫌疑人本人发出的消息，二者可以相互印证。
- 最终答案：`17706223971`

### 题目11：请分析检材1：嫌疑人使用浏览器访问了该下载链接，该浏览器对应的包名是什么？

- 题目原文：`请分析检材1：嫌疑人使用浏览器访问了该下载链接，该浏览器对应的包名是什么？`
- 证据类型：`手机取证 / 浏览器历史`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/browser_downloads/data/com.meiit.browser/databases/stock_assistant.db`；`exports/browser_downloads/user/0/com.meiit.browser/databases/stock_assistant.db`
- 关键证据/关键路径：表 `history`，`id=1`、`id=2`、`id=6`
- 提取/查询方法：检索 `url like '%pan.baidu.com%'`
- 重要截图：无
- 摘录：
...
- 推理：命中百度网盘下载链接的历史数据库路径属于应用 `com.meiit.browser`，因此对应浏览器包名就是 `com.meiit.browser`。
- 最终答案：`com.meiit.browser`

### 题目12：请分析检材1：嫌疑人于何时下载数据商人发送的数据压缩包文件？

- 题目原文：`请分析检材1：嫌疑人于何时下载数据商人发送的数据压缩包文件？`
- 证据类型：`手机取证 / 下载文件时间`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：原始 tar 条目 `media\0\Download\BaiduNetdisk\方象.zip`；本地抽取文件 `exports/selected_from_tar/media/0/Download/BaiduNetdisk/数据.zip`
- 关键证据/关键路径：tar 列表中的时间戳；抽取文件 `LastWriteTime`
...
- 推理：原始 tar 条目和本地抽取文件的修改时间一致，均为 `2026-05-13 18:37:56`，可作为该压缩包的下载时间。
- 最终答案：`2026-05-13 18:37:56`

### 题目13：请分析检材1：接上题，该压缩包的解压密码是什么？

- 题目原文：`请分析检材1：接上题，该压缩包的解压密码是什么？`
- 证据类型：`手机取证 / 聊天记录与压缩包验证`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/app_uni04963c4/data/uni.app.UNI04963C4/files/1600123285_3137373036323233393731/msg_0.db`；`exports/selected_from_tar/media/0/Download/BaiduNetdisk/数据.zip`
- 关键证据/关键路径：表 `message`，`id=17`；`userInfo.userID`
- 提取/查询方法：读取聊天原文，并执行 `7z.exe t -p17706223971 数据.zip`
- 重要截图：无
- 摘录：
...
- 推理：对方说明“解密密码是你的手机号码”，而上一题已确定嫌疑人聊天工具用户 ID 为 `17706223971`，使用该值测试压缩包后验证成功。
- 最终答案：`17706223971`

### 题目14：分析检材1：接上题，年龄在 40 至 60 岁（含 40、60 岁）的富豪共有多少人？

- 题目原文：`分析检材1：接上题，年龄在 40 至 60 岁（含 40、60 岁）的富豪共有多少人？`
- 证据类型：`手机取证 / 表格数据统计`
- 检材来源：`K:\检材1.tar`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\检材1`
- 证据来源：`exports/downloads_unpacked/data_zip_17706223971/数据.xlsx`
- 关键证据/关键路径：`Sheet1`；表头 `姓名、身份证号码、银行卡(借记卡)、手机号码、出生日期、年龄、出生地`
...
- 推理：工作簿共有 100 条数据，按年龄列筛选 `40` 到 `60` 岁（含边界）后统计结果为 `46`。
- 最终答案：`46`

### 题目15：综合分析：请找出嫌疑人筛选的重点客户名单，并统计名单内共有多少人？

- 题目原文：`综合分析：请找出嫌疑人筛选的重点客户名单，并统计名单内共有多少人？`
- 证据类型：`手机取证 + 计算机取证 / 分卷压缩与表格恢复`
- 检材来源：`K:\检材1.tar`、`K:\检材2.E01`；本地分析目录 `C:\Users\glj07\Desktop\Codex工作区\Writeup\Forensic\2026獬豸杯\综合_重点客户`
- 证据来源：`检材1/exports/stego/important_jpg/重点客户.z01`；`检材2/exports/suspicious_files/recycle_RC12AM0.zip`；`检材2/exports/suspicious_files/recycle_IC12AM0.zip`；`综合_重点客户/exports/combined_zip/extracted/重点客户.xlsx`
- 关键证据/关键路径：`重点客户.z01` 与 `重点客户.zip` 组合形成完整分卷压缩包；解压后得到 `重点客户.xlsx`
- 提取/查询方法：手机隐写文件提供第 1 卷 `重点客户.z01`；PC 回收站文件 `$RC12AM0.zip` 提供第 2 卷 `重点客户.zip`；组合后用 7z 解压完整工作簿
- 重要截图：无
- 摘录：
...
- 推理：单独查看 PC 端回收站压缩包会提示缺失分卷，说明其不是完整文件；将手机隐写提取到的 `重点客户.z01` 与回收站中的 `重点客户.zip` 组合后，7z 能完整解出 `重点客户.xlsx`，统计有效数据行为 5 条。
- 最终答案：`5`
