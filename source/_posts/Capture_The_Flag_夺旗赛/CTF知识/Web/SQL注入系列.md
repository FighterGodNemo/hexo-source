...
加入表设计时只能插入30字符长度，但你插入一个 admin（然后非常长空格后）1  真正插入的就是admin。如果表没有对这个插入后的值进行检测，那么你就能利用新插入的admin密码登录。
原表设计者可能设了 UNIQUE 约束或权限字段不可为 admin，攻击者通过闭合引号把 admin 和 1 硬塞进两列，绕过了"普通人只能注册普通账号"的业务约束。
![](SQL注入系列/file-20260803124519439.png)
