...
这是经典的**VARCHAR 截断 + PAD SPACE 约束绕过**。
加入表设计时只能插入x个字符长度，但如果你插入的是：admin + 大量空格 + 1，长度超过 VARCHAR(x) 后被截断，尾部的空格和 1 全丢了，数据库实际存的就是 admin。而 WHERE username='admin' 比较时又忽略尾部空格，于是和原 admin 行撞上了 UNIQUE 约束——如果约束没设好或者 INSERT ... ON DUPLICATE KEY 之类，就绕过注册限制。

# 报错注入

![](SQL注入系列/file-20260803135428135.png)